...
def get_alternative_route(road_name):
    ...
    # Cek apakah ada rute alternatif tetap yang ditentukan
    alt_name = road_alternatives.get(road_name)
    if not alt_name:
        return jsonify({'message': 'Tidak ada rute alternatif yang ditentukan untuk jalan ini.'})

    # Ambil info status jalan alternatif
    alt = next((r for r in status_data if r['name'] == alt_name), None)

    if not alt:
        return jsonify({'message': 'Rute alternatif tidak ditemukan dalam data sistem.'})

    # Jika jalan alternatif juga macet, beri peringatan
    ...
    # Fungsi untuk menghitung jarak kasar antar koordinat
